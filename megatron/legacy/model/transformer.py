...
class ParallelMLP(MegatronModule):
    ...
    def forward(self, hidden_states,ln_fun = None):
        ...
        # [s, b, 4hp]
        intermediate_parallel, bias_parallel = self.dense_h_to_4h(hidden_states,ln_fun = ln_fun)
        ...
        if self.bias_gelu_fusion:
            ...
            def act_fun(inps):
                return bias_gelu_impl(inps, bias_parallel)
            # intermediate_parallel_out = bias_gelu_impl(intermediate_parallel, bias_parallel)
        else:
            def act_fun(inps):
                return self.activation_func(inps)
            # if bias_parallel is not None:
            #     intermediate_parallel = intermediate_parallel + bias_parallel
        
            # intermediate_parallel_out = self.activation_func(intermediate_parallel)

        # [s, b, h]
        output, output_bias = self.dense_4h_to_h(intermediate_parallel,act_fun = act_fun)
...
class ParallelAttention(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask,
                ...
                rotary_pos_emb=None,ln_fun = None):
        ...
        if self.attention_type == AttnType.self_attn:
            ...
            mixed_x_layer, _ = self.query_key_value(hidden_states,ln_fun = ln_fun)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        def input_ln_fun(inps):
            return self.input_norm(inps)
        # Self attention.
        attention_output, attention_bias = \
            self.self_attention(
                hidden_states,
        ...
        if self.drop_path is None:
            ...
            if attention_bias is None and self.hidden_dropout <=1e-5:
        ...
        # MLP.
        def post_att_ln_fun(inps):
            return self.post_attention_norm(inps)
        mlp_output, mlp_bias = self.mlp(norm_input,ln_fun=post_att_ln_fun)
