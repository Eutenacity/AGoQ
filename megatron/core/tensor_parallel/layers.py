...
class OffloadReload(torch.autograd.Function):
    ...
    def forward(ctx,x,hidden_state):
        ...
        if OffloadReload.gpu_storage is None:
            # OffloadReload.cpu_storage=torch.UntypedStorage(8192*11008*3).pin_memory(ctx.device)
...
class CustomLN_Pre(torch.autograd.Function):
    ...
    def backward(ctx,grad):
        ...
        torch.autograd.backward(ctx.share_list[2], grad_tensors=grad)
        out_grad = ctx.share_list[1].grad
...
class CustomLN_Post(torch.autograd.Function):
    ...
    def backward(ctx,grad):
        ...
        with torch.enable_grad():
            ctx.share_list[2] = ctx.ln_fun(ret)
        ctx.hidden_state.data.copy_(ctx.share_list[2])
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_: torch.Tensor, weight: Optional[torch.Tensor] = None, ln_fun= None):
...
class CustomACT_Pre(torch.autograd.Function):
    ...
    def backward(ctx,grad):
        ...
        # ctx.share_list[2].backward(grad)
        torch.autograd.backward(ctx.share_list[2], grad_tensors=grad)
        out_grad = ctx.share_list[1].grad
        ctx.share_list[0] = None
        ctx.share_list[1] = None
...
class CustomACT_Post(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx,output, hidden_state,act_fun,share_list):
        ctx.share_list = share_list
        ctx.act_fun = act_fun 
        ctx.device = hidden_state.device
    ...
    def backward(ctx,grad):
        ...
        ctx.share_list[1] = ret
        with torch.enable_grad():
            ctx.share_list[2] = ctx.act_fun(ret)
        ctx.hidden_state.data.copy_(ctx.share_list[2])
...
class RowParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_,act_fun = None):
